...
def record(form: Path) -> Path:
    ...
    print(f"asking {settings.model} about {schema.name} "
          f"({len(schema.fields)} fields)…", file=sys.stderr)
    response = client.transport.send(request)

    out = FIXTURES / f"rules_{schema.name}.json"
    out.write_text(json.dumps({
        "_note": (
            "Recorded from the real API. Re-record with livetests/record.py "
            "when a prompt in fillerai/llm/prompts.py changes."
        ),
        "form": schema.name,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        raise SystemExit(2)
    print(f"wrote {record(Path(sys.argv[1]))}", file=sys.stderr)
